...
export default function Logo({ size = "md" }: { size?: Size }) {
  ...
  return (
    <View style={[styles.row, { gap: s.gap }]} testID="app-logo">
      ...
      <View style={[styles.iconWrap, { width: s.icon + 4, height: s.icon + 4, marginLeft: 14 }]}>
        <Ionicons name="book" size={s.icon} color={colors.copper} style={{ position: "absolute", top: 2, left: 2, ...(Platform.OS === "web" ? { filter: `drop-shadow(0 0 ${s.glow}px ${colors.copper})` as any } : {}) }} />
        <Ionicons name="book" size={s.icon} color={colors.brass} style={{ position: "absolute", top: 0, left: 0, opacity: 0.85, ...(Platform.OS === "web" ? { filter: `drop-shadow(0 0 ${s.glow}px ${colors.brass})` as any } : {}) }} />
      </View>
    </View>
  );
}
