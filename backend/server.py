...
async def guest_login(req: Optional[GuestLoginRequest] = None):
    device_id = req.device_id if req else None

    # Si ya conocemos este dispositivo (device_id persistente, guardado
    # en el móvil y que sobrevive a cerrar sesión), reutilizamos la MISMA
    # cuenta invitada de siempre, con sus límites diarios ya gastados si
    # correspondía — en vez de crear una cuenta nueva y "resetear" el
    # contador cada vez que alguien cierra sesión y vuelve a entrar.
    if device_id:
        existing = await db.users.find_one({"device_id": device_id, "email": {"$regex": "^guest_"}}, {"_id": 0})
        if existing:
            user_id = existing["user_id"]
            ...
            return {"user": User(**existing).dict(), "session_token": session_token}
...
def _build_narrator_system_prompt(book: dict) -> str:
    ...
    return (
        ...
        f"ESTILO — REGLA MÁS IMPORTANTE DE TODAS: escribes como un mensaje de WhatsApp. "
        f"LÍMITE ABSOLUTO: máximo 35 palabras por respuesta. Cuenta mentalmente: si tu "
        f"respuesta supera las 35 palabras, bórrala y escribe una versión más corta. "
        f"Nada de párrafos largos ni de desarrollar varias ideas seguidas — elige solo "
        f"la más importante y dila en pocas palabras. Esta regla tiene prioridad sobre "
        f"sonar completo o dar contexto de más.\n\n"
        f"CONOCIMIENTO Y CONTENIDO ESPECÍFICO: solo conoces lo que dice la sinopsis que "
        f"tienes arriba. Si te piden la solución, respuesta, resumen o contenido concreto "
        f"de un capítulo, apartado, ejercicio o sección específica del libro (por ejemplo "
        f"\"dame la solución del apartado 4\" o \"resume el capítulo 3\"), NUNCA construyas "
        f"una respuesta que suene a que conoces ese contenido exacto, aunque creas que "
        f"puedes deducirlo del tema general del libro — eso sería inventar. En su lugar, "
        f"responde corto y honesto: reconoce que no tienes ese detalle tan concreto y "
        f"anima a leer esa parte directamente. Nunca disfraces una suposición como si "
        f"fuera un dato real del libro.\n\n"
        f"Adapta tu energía al tono del usuario: si te preguntan con curiosidad y calidez, "
        f"ábrete más; si son más directos o escuetos, sé tú también más conciso.\n\n"
